...
function retornaMensagem() {
    var mensagem = '';
    mensagem = "<td>" + arr[arr.length - 1].codigo + "</td>" + 
                "<td>" + arr[arr.length - 1].nome + "</td>" + 
                "<td>" + arr[arr.length - 1].email + "</td>" + 
                '<td style=" background: none; border: none; ">' + 
                '<button onclick="escolheARR(event)" class="button btnApagaLinha">' + arr[arr.length - 1].codigo + '</button>' + 
                '</td>';
...
function procuraIndexDoArr() {
    for( var i=0; i<arr.length; i++) if( $btn_content === arr[i].codigo ) return i;
}

function apagatr() {
    $table_body.removeChild($table_body.childNodes[index_tr+1]);
    contaNumerolinhas();
}

function filtraArray() {
    arr = arr.filter(function( el ){
        return el.codigo !== $btn_content;
    });   
}
