...
function retornaMensagem() {
    ...
    mensagem =  "<td class='td_body'>" + arr[arr.length - 1].codigo + "</td>" + 
                ...
                '<button onclick="escolheARR(event)" class="btn btn-secondary" codigoID="' + arr[arr.length - 1].codigo + '">Excluir</button>' + 
                '</td>' +
                '<td style=" background: none; border: none; ">' + 
                '<button onclick="openModal(event)" type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#staticBackdrop" codigoID="' + arr[arr.length - 1].codigo + 
