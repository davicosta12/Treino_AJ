(function(){
    ...
    var $btnLimpaLinha = document.getElementsByClassName('btnApagaLinha');
    ...
    function addItemNaTabela() {
        ...
        $table_body.innerHTML += "<td>" + arr[arr.length - 1].codigo + "</td>" + 
                                "<td>" + arr[arr.length - 1].nome + "</td>" + 
                                "<td>" + arr[arr.length - 1].email + "</td>" + 
                                '<td style=" background: none; border: none; ">' + 
                                '<button class="button btnApagaLinha onclick="filtraArr()"">' + arr[arr.length - 1].codigo + '</button>' + 
                                '</td>';

        console.log(arr);
        console.log($btnLimpaLinha);
        console.log($trow_body);
    ...
    for(var i=0; i<$btnLimpaLinha.length; i++) {
        $btnLimpaLinha[i].addEventListener('click', filtraArr);
    }

    function filtraArr(arr, $table_body) {
        console.log('clickei');
    }
